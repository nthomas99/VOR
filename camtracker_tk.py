...
class Setup:
	...
	def setup_GUI(self):
		...
		# find image paths
		imgpaths = {}
		buttnames = ['1','2','3','4','up','down','t','space','r','escape']
		...
		buttpos['4'] = int(self.dispsize[0]*(5/6.0) - buttsize[0]/2), y
		buttpos['space'] = int(self.dispsize[0]*(6/6.0) - buttsize[0]/2), y
	...
	def draw_stage(self, stagenr=None):
		...
		# universal buttons
		buttonstodraw = ['1','2','3','4','space','escape','t','r']
		activetodraw = []
		...
		while running:
			...
			if stagevars[3]['confirmed']:
				...
				while loop_counter < 1000:
					useprect = stagevars[0]['use_prect'] and stage > 1
					self.img, self.thresholded, pupilpos, pupilsize, pupilbounds = self.tracker.give_me_all(pupilrect=useprect)
					eye_timestamp = time.time()
					if 1:   										#Change to '0' to stop printing time
						eye_data.append(str("timestamp,%s" % (eye_timestamp)) + ","),
					if 1:											#Change to '0' to stop printing pupilpos
						pupilpos_str = ','.join(map(str,pupilpos))	#Converts pupilpos tuple into string
						eye_data.append(str("pupilpos,%s" % (pupilpos_str)) + ","),
					# update settings
	...
	def handle_input(self, inptype, inp, stage, stagevars):
		...
		if inp == 'escape':
			...
			with open("eye_file.txt", "w") as eye_file:
				eye_file.write(str(eye_data))
			print "saved, bye"
			pygame.display.quit()
